...
@interface YYPublishBaseViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,JJPhotoDelegate,YYPublishBaseViewDelegate>{
...
@implementation YYPublishBaseViewController
...
-(void)initPickerView{
    ...
    //添加图片提示
    _addImageStrLabel = [[UILabel alloc]initWithFrame:CGRectMake(100, 50, 70, 20)];
    _addImageStrLabel.text = @"添加图片";
    _addImageStrLabel.font = [UIFont systemFontOfSize:14];
    _addImageStrLabel.textColor = [UIColor colorWithRed:153.0/255.0 green:153.0/255.0 blue:153.0/255.0 alpha:1.0];
    [self.pickerCollectionView addSubview:_addImageStrLabel];
...
- (void)deletePhoto:(UIButton *)sender{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:kSelectedPhotosChangeNotification object:nil];
    //没有任何图片
    if (_imageArray.count == 0) {
        _addImageStrLabel.hidden = NO;
    }
